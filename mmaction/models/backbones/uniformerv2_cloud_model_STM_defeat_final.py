...
class UniFormerV2_CloudModel(BaseModule):
    ...
    def __init__(
        ...
        # self.feature_pool = nn.MaxPool3d(kernel_size=(1,2,2))
        # self.edge_feat_conv = nn.Sequential(
        ...
        # )
        # v3 STM
        self.down_sample_1x1_conv = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=12, kernel_size=1, bias=False))
        self.cstm_channel_wise_conv = nn.Sequential(nn.Conv2d(in_channels=8, out_channels=8, kernel_size=1, groups=8, bias=False),
                                                    nn.ReLU6(inplace=True)) 
        self.local_spatial_conv = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=24, kernel_size=3, padding=1, bias=False),
                                                nn.ReLU6(inplace=True))
        self.cmm_channel_wise_conv = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=24, kernel_size=3, stride=1, padding=1, groups=24, bias=False),
                                                   nn.ReLU6(inplace=True))
        self.fusion_conv_1x1 = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=24, kernel_size=1, bias=False),
                                             nn.ReLU6(inplace=True))
        # self.edge_feat_conv = nn.Sequential(
        #   conv_1x1_bn(24, 48), # 94 56 56
        #   conv_1x1_bn(48, 96), # 94 56 56
        #   nn.Conv2d(96, 384, (patch_size // 8, patch_size // 8), (patch_size // 8, patch_size // 8), (0, 0), bias=False), # 384 14 14
        #   nn.ReLU6(inplace=True),
        #   conv_1x1_bn(384, 768), # 768 14 14
        # )
        self.edge_feat_conv = nn.Conv3d(
                24,
                width, (1, patch_size // 4, patch_size // 4),
                (1, patch_size // 4, patch_size // 4), (0, 0, 0),
                bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.EdgeModel is not None:
            ...
            q = x[3].cpu().detach().numpy().tolist()
            x = x[2]

            # STM
            # Channelwise SpatioTemporal Module
            feat = x.clone()    # torch.Size([128, 24, 56, 56])
            NT, C, H, W = x.shape # bs 768 8(6) 14 14     # torch.Size([bs, 768, 8, 14, 14])
            T = 8
            x = x.reshape(NT//T, C, T, H, W)
            residual = x.clone()
            N, C, T, H, W = x.shape
            x = x.reshape(T, N*H*W, C)
            x = self.cstm_channel_wise_conv(x)
            for i in range(T):
                if i > 0:
                    # pre = x[i, :, :]
                    # x[i, :, :] = x[i, :, :] + pre
                ...
                if i < T-1:
                    ...
                    after = x[i+1]
                    x[i] = x[i] + after
            x = x.reshape(N*T, C, H, W) # torch.Size([128, 24, 56, 56])
            x = self.local_spatial_conv(x)  # torch.Size([128, 24, 56, 56])
            # CMM
            feat_conv = self.cmm_channel_wise_conv(feat.clone())    # torch.Size([128, 24, 56, 56])
            NT, C, H, W = feat.shape # bs 768 8(6) 14 14     # torch.Size([bs, 768, 8, 14, 14])
            T = 8
            feat = feat.reshape(NT//T, C, T, H, W)
            N, C, T, H, W = feat.shape
            feat = feat.reshape(T, N*H*W, C)
            feat_conv = feat.reshape(T, N*H*W, C)
            for i in range(T-1):
                feat[i] = feat_conv[i+1] - feat[i]
            feat[7] = torch.zeros_like(feat[7]) - feat[7]
            feat = feat.reshape(N*T, C, H, W)
            x = x + feat
            x = self.fusion_conv_1x1(x)
            x = x.reshape(N, C, T, H, W)    # torch.Size([16, 24, 8, 56, 56])
            x = x + residual
            edge_feats = x
            # nt, 24, 56, 56 的特征处理
            x = self.edge_feat_conv(x)  # torch.Size([16, 768, 8, 28, 28])
            # N, C, T, H, W = x.shape
        ...
        # edge_patch = None
        # cloud_patch = None
        # for feature regulization
        edge_patch = x
        # with torch.no_grad():
        #     cloud_patch = self.conv1(inputs.reshape(-1, 8, 3, 224, 224).permute(0, 2, 1, 3, 4))
        #     N, C, T, H, W = cloud_patch.shape # bs 768 8(6) 14 14
        #     cloud_patch = cloud_patch.permute(0, 2, 3, 4, 1).reshape(N * T, H * W, C)   # bs * 8(6), 14 * 14, 768

        # residual add cloud patch(just try)
        # cloud_patch = self.conv1(inputs.reshape(-1, 8, 3, 224, 224).permute(0, 2, 1, 3, 4))
        # N, C, T, H, W = cloud_patch.shape # bs 768 8(6) 14 14
        # cloud_patch = cloud_patch.permute(0, 2, 3, 4, 1).reshape(N * T, H * W, C)   # bs * 8(6), 14 * 14, 768
        # x = x + cloud_patch
