...
class UniFormerV2_CloudModel_Raw_Stage_2_Feat(BaseModule):
    """UniFormerV2:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.EdgeModel is not None:
            ...
            # print(x)
            # top_singular_value = 16
            # for i in range(x.shape[0]):
            #     video_i = x[i].squeeze(0)
            #     for j in range(video_i.shape[0]):
            #         frame_j = video_i[j].squeeze(0)
            #         # frame_j = frame_j.reshape(C, H*W)
            #         frame_j = frame_j.reshape(C*H, W)
            #         U, S, Vh = torch.linalg.svd(frame_j, full_matrices=False)
            #         # print(S)
            #         recons = U[:, :top_singular_value] @ torch.diag(S[:top_singular_value]) @ Vh[:top_singular_value, :] # torch.mm(torch.mm(U, torch.diag(S)), Vh)
            #         recons = recons.transpose(0, 1)
            #         video_i[j] = recons.reshape(C, H, W).unsqueeze(0)
            #     x[i] = video_i.unsqueeze(0)
            # x = x.reshape(NT, C, H, W)
            if self.svd_top_ratio != 1.0:
                for i in range(x.shape[0]):
                    video_i = x[i].squeeze(0)
                    ...
                    # print(S)
                    # print(torch.sum(S))
                    # print(torch.sum(S) * 0.95)
                    # print('----------------------------{}'.format(self.svd_top_ratio))
                    for j in range(S.shape[0]):
                        if torch.sum(S[:j]) > torch.sum(S) * save_ratio:
            ...
            #         print(Vh.shape)
            #         print(S)
            #         for k in range(S.shape[0]//2, S.shape[0]):
            #             S[k] = 0.
            #         print(S)
            #         recons = torch.mm(torch.mm(U, torch.diag(S)), Vh)
            #         print(recons)
            #         print(recons.shape)
