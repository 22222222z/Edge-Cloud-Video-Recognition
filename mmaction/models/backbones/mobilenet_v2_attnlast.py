...
class MobileNetV2_attnlast(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.layers):
            ...
            if i == 2:
                stage_2_feat = x
                if hasattr(self, 'svd_top_ratio') and self.svd_top_ratio != 1.0:
                    # save_ratio = 0.6
                    save_ratio = self.svd_top_ratio
                    NT, C, H, W = x.size()
        ...
        # mem = x.view(n_batch, n_segment, c, h, w).permute(1, 3, 4, 0, 2).contiguous().view(-1, n_batch, c)
        # q = self.global_block[idx](q, mem)
        # obtain CNN prediciton
        # print(x.shape)    # 128 x 1280 x 7 x 7
        feat = x
        x = x.mean(3).mean(2)# BxT, C
        # print(x.shape)    # 128 x 1280
        ...
        # print('q.shape: {}'.format(q.shape))    # 128 x 1280
        ...
        # return tuple(outs)
        return [feat, q, low_feat, stage_2_feat, stage_4_feat, stage_3_feat]
    ...
    def train(self, mode=True):
        ...
        super(MobileNetV2_attnlast, self).train(mode)
        self._freeze_stages()
