...
class UniFormerV2_CloudModel(BaseModule):
    ...
    def __init__(
        ...
        # )
        # v3 STM
        self.cstm_channel_wise_conv = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=1, groups=8)
        self.cmm_channel_wise_conv = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=3, stride=1, padding=1, groups=8)
        self.fusion_conv_1x1 = nn.Conv2d(in_channels=8, out_channels=8, kernel_size=1)
        self.edge_feat_conv = nn.Sequential(
          conv_1x1_bn(12, 48), # 94 56 56
          conv_1x1_bn(48, 96), # 94 56 56
          nn.Conv2d(96, 384, (patch_size // 8, patch_size // 8), (patch_size // 8, patch_size // 8), (0, 0), bias=False), # 384 14 14
        ...
        if self.EdgeModel is not None:
            ...
            for i in range(T):
                ...
                if i < T-1:
                    ...
                    x[i] = x[i] + after
            # CMM
            feat = self.cmm_channel_wise_conv(feat)
            ...
            edge_feats = x
            # nt, 24, 56, 56 的特征处理
            x = self.edge_feat_conv(x)  #x.shape: torch.Size([64, 768, 14, 14])
