...
class UniFormerV2_CloudModel(BaseModule):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.EdgeModel is not None:
            # print(x.shape)  # torch.Size([20, 1, 8, 3, 224, 224])
            # n, z, t, c, h, w = x.shape
            ...
            nt, c, h, w = x.shape
            t = 8
            x = x.reshape(nt//t, t, c, h, w).permute(1, 0, 2, 3, 4) # torch.Size([8, 16, 1280, 7, 7])
            buffer_x = x.clone()
            buffer_x = buffer_x.mean(2).unsqueeze(2).repeat(1, 1, 768, 2, 2)   # torch.Size([8, 16, 1280, 14, 14])

            # use deconv to upsample TSM feature x
            conv_x_list = []
            for i in range(x.shape[0]):
                conv_x_list.append(self.deconv1(x[i].squeeze(0)))
            # conv_x = torch.zeros(conv_x_list[0].shape).unsqueeze(0).repeat(8, 1, 1, 1, 1)   # torch.Size([8, 16, 768, 14, 14])
            for i in range(len(conv_x_list)):
                buffer_x[i] = conv_x_list[i]
            x = buffer_x  # torch.Size([8, 16, 768, 14, 14])
            t, n, c, h, w = x.shape
