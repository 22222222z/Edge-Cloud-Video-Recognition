...
class UniFormerV2_CloudModel_Raw_Stage_4_Feat(BaseModule):
    """UniFormerV2:
    ...
    def __init__(
        ...
        # v1
        self.edge_feat_conv = nn.Sequential(
        #   conv_1x1_bn(32, 96), # 94 56 56
          nn.Conv2d(96, 384, (patch_size // 16, patch_size // 16), (patch_size // 16, patch_size // 16), (0, 0), bias=False), # 384 14 14
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.EdgeModel is not None:
            if x.size()[-1] != 56:
                ...
                # print(self)
                # exit(0)
                edge_scores = self.EdgeModel.cls_head(x, num_segments=8)
                # q = x[3].cpu().detach().numpy().tolist()
                q = x[1].cpu().detach().numpy().tolist()
                x = x[4]
            # edge_final_feats = x
            # edge_scores = None
            ...
            #     ll_q = []
            #     for j in range(len(q[0])//2):
            #         ll_q.append(q[i].index(max(q[i])))
            #         # assert q[i].index(max(q[i])) < 12, 'index larger than 12'
            #         q[i][q[i].index(max(q[i]))] = 0
            #     l_q.append(ll_q)
            ...
            # nt, c, h, w = x.shape
            # filtered_x = x.clone()[:, :12, :, :]
            # temp_x = x.clone()
            # # filtered_x = torch.randn(nt, c//2, h, w, device=x.device, requires_grad=True)
            # # x = x.cpu().detach()
            # for i in range(filtered_x.size()[0]):
            #     for j in range(len(l_q[i])):
            #         filtered_x[i][j] = temp_x[i][l_q[i][j]]
            # x = filtered_x
        ...
        return [out, edge_final_feats, edge_patch, edge_scores]
