...
class UniFormerV2_CloudModel(BaseModule):
    ...
    def __init__(
        ...
        # )
        # v3 STM
        self.down_sample_1x1_conv = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=12, kernel_size=1))
        self.cstm_channel_wise_conv = nn.Sequential(nn.Conv2d(in_channels=8, out_channels=8, kernel_size=1, groups=8),
                                                    nn.ReLU6(inplace=True)) 
        self.local_spatial_conv = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=24, kernel_size=3, padding=1),
                                                nn.ReLU6(inplace=True))
        self.cmm_channel_wise_conv = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=24, kernel_size=3, stride=1, padding=1, groups=24),
                                                   nn.ReLU6(inplace=True))
        self.fusion_conv_1x1 = nn.Sequential(nn.Conv2d(in_channels=24, out_channels=24, kernel_size=1),
                                             nn.ReLU6(inplace=True))
        # self.edge_feat_conv = nn.Sequential(
